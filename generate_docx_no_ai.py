...
doc.add_paragraph(
    "In its enhanced architecture, the platform utilizes Google Spreadsheets as a dynamic, centralized Data Backend. This allows network engineers and on-ground staff to update network metrics and logging dynamically without requiring database redeployments. The platform translates this raw data into actionable insights for rapid network triage."
)
...
p = doc.add_paragraph()
p.add_run('Frontend Components: ').bold = True
p.add_run('Includes the Geospatial Leaflet Map, the live-updating Alerts Sidebar, Recharts for analytics, and dynamic Data Tables.')
p.style = 'List Bullet'
...
doc.add_paragraph("6. The Client UI recalculates the Severity Score (Complaint-to-Subscriber Ratio).", style='List Number')
doc.add_paragraph("7. The UI re-ranks the Alerts Sidebar to bubble up the most critical issues and updates the Map Node colors (e.g., turning a node Red for Critical).", style='List Number')
...
doc.save('Project_ALO_Architecture_No_AI.docx')
print("Detailed Word document without AI created successfully.")
