...
def download_kroki_diagram(diagram_text, filename):
...
diagram1 = """graph TB
    subgraph Data Layer [Google Workspace Backend]
        GS[Google Spreadsheet\n'Network & Complaints Data']
        GAPI[Google Sheets API v4]
        GS --> GAPI
    end

    subgraph Backend Layer [Next.js Serverless API]
        NR[Next.js API Routes]
        Cache[(In-Memory / Vercel KV Cache)]
        Transformer[Data Transformation Service]
        GAPI <-->|OAuth/Service Account| NR
        NR --> Cache
        NR --> Transformer
    end

    subgraph Frontend Layer [React Client]
        Ctx[Dashboard Context API]
        Transformer -->|JSON Payload| Ctx
        
        subgraph UI Components
            Map[Geospatial Map\nLeaflet]
            Alerts[Alerts Sidebar\nLive Ranking]
            Charts[Analytics Dashboard\nRecharts]
            Insights[Predictive Insights Panel]
        end
        Ctx --> Map & Alerts & Charts & Insights
    end

    subgraph AI/ML Layer [Future Integration]
        AIEngine[AI Inference Engine]
        Model[(Predictive Models)]
        AIEngine -->|Polls for Updates| GS
        AIEngine <--> Model
        AIEngine -->|Pushes Insights| NR
    end
"""
...
diagram2 = """sequenceDiagram
    ...
    participant API as Next.js API / Sheets API
    participant Cache as Edge Cache
    participant Client as Dashboard UI
    participant AI as AI Engine (Future)

    NOC->>Sheet: Updates complaint count for OLT "Hyderabad"
    Sheet-->>Sheet: Auto-saves data
    Client->>API: Periodic Polling / Refresh Request
    API->>Cache: Check Cache validity
    alt Cache Expired
        API->>Sheet: Fetch latest row data via Sheets API
        Sheet-->>API: Returns updated 2D array
        API->>API: Transform to Hierarchical JSON
        API->>Cache: Update Cache
    end
    API-->>Client: Returns updated network payload
    
    Client->>Client: Calculate Severity (Complaint-to-Subscriber Ratio)
    Client->>Client: Re-rank Alerts Sidebar
    Client->>Client: Update Map Node Color (e.g., Red for Critical)
    
    par AI Processing
        AI->>Sheet: Ingest historical data
        AI->>AI: Run Anomaly Detection Model
        AI->>API: Push predictive failure alert
        API-->>Client: Display "High Risk of OLT Failure" Warning
    end
"""

download_kroki_diagram(diagram1, "diagram1.png")
download_kroki_diagram(diagram2, "diagram2.png")
...
if os.path.exists('diagram1.png'):
    doc.add_picture('diagram1.png', width=Inches(6.0))
...
if os.path.exists('diagram2.png'):
    doc.add_picture('diagram2.png', width=Inches(6.0))
...
doc.save('Detailed_Project_ALO_Architecture_With_Diagrams.docx')
print("Detailed Word document with diagrams created successfully.")
