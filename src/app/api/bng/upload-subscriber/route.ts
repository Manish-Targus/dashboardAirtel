...
const SHEET_NAME = 'Subscriber Hourly Trend Report';

// Col indices in the subscriber sheet
// 0:VendorName 1:Circle 2:City 3:LinkType 4:NodeName 5:ParentIface 6:ActiveIface 7:BRASType
// 10:PortCapacity 11-34:HOUR_00-23 35:MaxSubscriber 36:MaxUtil(%) 37:Category
...
function validate(rows: unknown[][]): ValidationError[] {
  ...
  const dateRow = rows.find(r => r[0] === 'Date:') as unknown[] | undefined;
  if (!dateRow) {
  ...
  if (header.length < 37) {
    errors.push({
      field: 'Columns',
      message: `Header has only ${header.length} column(s). Need at least 37 (up to "Max Util(%)" in column AK).`,
    });
  }
  ...
  const dataRows = rows.slice(headerIdx + 1).filter(r => r[4]);
...
export async function POST(request: Request) {
  try {
    ...
    const nodeMap = new Map<string, {
      ...
      ae_interfaces: { name: string; link_type: string; bw_gb: null; max_util: number; port_capacity?: number }[];
    }>();

    for (const r of rows.slice(headerIdx + 1)) {
      if (!r[4]) continue;
      const ifaceName = String(r[5] ?? '').trim();
      if (!ifaceName.startsWith('ae')) continue;
      ...
      if (!nodeMap.has(nodeName)) {
        nodeMap.set(nodeName, {
          ...
          circle:    String(r[1] ?? ''),
          city:      String(r[2] ?? ''),
          bras_type: String(r[7] ?? ''),
          ae_interfaces: [],
        });
      }
      const rawUtil = Number(r[36]);
      const rawCap  = Number(r[10]);
      nodeMap.get(nodeName)!.ae_interfaces.push({
        name:          ifaceName,
        link_type:     String(r[3] ?? ''),
        bw_gb:         null,
        max_util:      isNaN(rawUtil) ? 0 : rawUtil,
        port_capacity: isNaN(rawCap) ? undefined : rawCap,
      });
    }
    ...
    if (nodes.length === 0) {
      return NextResponse.json(
        { error: 'No usable data found.', details: ['Parsed 0 nodes. Check column F (Parent Interface) contains ae* bundle interface entries.'] },
        { status: 422 }
      );
    }
